...
Outputs:
  data/dataSet.json              — full merge (generated; input is SOURCE_FILES)
  ...
  data/dist/cities/{縣市}.json    — full records per city (for the 全部 tab)
...
def write_mobile_registry(cell_keys: list[str], city_names: list[str]) -> None:
    lines = [
        ...
        "/* Last-resort hotspot only; full data is CDN + disk cache */",
        "",
        "const loaders = {",
    ]
...
def main() -> None:
    ...
    print(
        ...
        f"cellSize={manifest['cellSize']}, version={manifest['version']})"
    )
