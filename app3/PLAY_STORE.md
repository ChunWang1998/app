...
```
附近狗主人交友・探索鄰汪夥伴・汪汪聚會・安全 Connect
```

### 完整說明

```
鄰汪幫狗主人找到附近同樣養犬的鄰居，約戶外見面、辦汪汪聚會。

【試用範圍】
・目前開放：臺北市、新北市、臺南市、高雄市
・以定位判斷你所在縣市（不能手選切換其他縣市）

【主要功能】
・探索鄰汪夥伴：時段篩選、行政區縮小範圍
・主人詳情與 Connect：雙方接受後解鎖聊天（最多 20 句）
・汪汪聚會：瀏覽／報名／創辦（需附 LINE 群組連結）
・創始 100 人：完成手機號＋狗檔案即可永久免費（不驗證碼）

【怎麼用】
1. 開啟 App，允許定位
2. 在試用縣市內瀏覽探索與聚會
3. 右上角完成手機號＋狗檔案（合照須主人與狗同框）
4. Connect 成功後雙方自行約公園見面

本 App 需定位與網路。聊天、檔案與白名單存在雲端（換機輸入同一手機號可還原）。
```
...
- 是否以資料為賣點／出售：否
- 是否強制收集非必要資料：否（定位為核心功能；註冊需手機號＋狗檔案）
...
- [x] `app.json` Android package `com.linwang.app`、定位／相簿／相機／通知權限
...
- [ ] 上傳商店素材與說明
- [ ] 填資料安全表單（v1 無 IAP）
- [ ] 上傳 AAB 至內部測試 → 真機測定位、註冊、Connect、聚會
- [ ] 再考慮正式版／iOS／v2 訂閱
